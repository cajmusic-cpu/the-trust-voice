...
export class TrustVoiceStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Short-lived tokens that map a 12-char alphanumeric code to a presigned S3 PUT URL.
    // Generated by the upload-link CLI tool; read by the unauthenticated /upload-url endpoint.
    ...
    // ── UPLOAD URL LAMBDA ─────────────────────────────────────────────────────
    //
    // Unauthenticated endpoint — videographers have no Cognito account.
    // Reads a short token from the query string and returns the presigned PUT URL
    // stored in UploadTokensTable. Token is written by the upload-link CLI tool.
    ...
    // GET /upload-url?token=TOKEN — unauthenticated, CORS for upload.thetrustvoice.com only
